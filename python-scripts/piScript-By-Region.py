...
# Filter by region
possible_regions = ["Africa", "CentralAmerica", "EastAsia", "Madagascar", "Melanesia", "MidEast", "SoutheastAsia", "SouthAmerica", "SouthAsia"]

# Make sure the user input is valid
if sys.argv[2] not in possible_regions:
    print("Please choose a valid region!")
    exit()


regionDNA = [] # empty list for filtering

for i in range(mtDNA_length):
    if mtDNA[i].id.startswith(sys.argv[2]):
        match = mtDNA[i]
        regionDNA.append(match)

regionDNA_length = len(regionDNA)

pi_list = [] # counter to add up all the pi values
loop_count = 0 # counter to keep track of the number of sequence comparisons

for index_1 in range(regionDNA_length):           # for loop to index the first sequence
    for index_2 in range(regionDNA_length):   # for loop to index the second sequence
        if index_1 > index_2:                 # excludes index_1 = index_2 AND any duplicate comparisons
            #print(index_1, index_2)
            seq_1 = regionDNA[index_1]            # rename the sequences for simplicity
            seq_2 = regionDNA[index_2]            
            ...
            {regionDNA[index_1]} and {regionDNA[index_2]}"""
...
print(f"The average value of pi for {loop_count} comparisons of {regionDNA_length} sequences from {sys.argv[2]} is:", pi_avg)
...
plt.xlabel('Pi Value')
plt.ylabel('Count')
plt.title(f"Histogram of Pi Values for sequence comparisons from {sys.argv[2]}")

plt.savefig(f"{sys.argv[2]}-pi-value-histogram.pdf") #can also add full path to save to a specific place
#plt.close() so it doesn't overlay the figures on top of each other when creating more than one plot
