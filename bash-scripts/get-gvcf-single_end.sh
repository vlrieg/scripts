...
if [ "$1" = '-h' ] ; then
  echo "Usage:"
  echo "   get-gvcf.sh <accessions-file> <ref>.fasta"
  echo "   get-gvcf.sh <accessions-file> (uses PVP01.fa)"
  exit 0
fi
...
#SBATCH --output=get-gvcf%A-%a.out    # Standard output and error log
#SBATCH --mem=60G
#SBATCH -c16
#SBATCH --job-name=get-gvcf-single-end.sh
...
if [ ! -e bam-constructed ] && [ ! -e bam-deduped ] ; then
  ...
  if [ ! -e "\${R3}" ] ; then 
    echo "Download finished, but can't find "\${R3}"?!  Quitting."
    exit 1
  fi
  ...
  echo bwa mem -M -t 32 "${REF}" \${R3} -R "\${RG}" \| samtools sort -@32 - -o "\${BAM}"
  bwa mem -M -t 32 "${REF}" \${R3} -R "\${RG}"| samtools sort -@32 - -o "\${BAM}"
...
# GVCF output step
\${RUN_GATK3} -T HaplotypeCaller -nct 4 -R ${REF} -I \${ACC}-recal_reads.bam --emitRefConfidence GVCF -o \${ACC}.g.vcf


#5. Clean up 

if [ -e "\${R3}" ] ; then
  echo -n "Removing reads \${R3}... "
  rm -f "\${R3}"
  echo done
fi
